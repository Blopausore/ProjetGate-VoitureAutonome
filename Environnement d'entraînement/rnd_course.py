...
class rnd_course(AbstractEnv):
    ...
    def _reward(self, action: np.ndarray) -> float:
        ...
        if 99 < self.vehicle.position[0] < 101 and -60 < self.vehicle.position[1]  <-40 :
            reward+=10
            self.vehicle.checkpoint +=1
        
        if self.vehicle.checkpoint > 3:
            print("trouvé")
            reward += 1000
    ...
    def _make_road(self):
        ...
        speedlimits = [None,10,10,10] #définit la vitesse max dans chaque section (je pense qu'on pourra mettre un très grand nombre après un minimum d'entrainement de l'ia)
        fin = (0,0)
        def trace(net,precedent,suivant,debut,fin) : 
            net.add_lane((precedent), (suivant), StraightLane(debut, fin, line_types=(LineType.CONTINUOUS, LineType.NONE), width=10, speed_limit=10))
            net.add_lane((precedent), (suivant), StraightLane(debut, fin, line_types=(LineType.STRIPED, LineType.CONTINUOUS), width=10, speed_limit=10))
            return fin[0] , fin [1]
        if random.random() > 0.5 :
            tourn = -1
        else : 
            tourn = 1
        x,y = trace(net,"a","b",(x,y),(x + random.random()*50, y ))
        #x, y= x-7.5 , y -7.5 * tourn
        x,y = trace(net,"b","c",(x,y),(x , y + tourn*(random.random()*5 + 15))) 
        #x, y= x+7.5 , y -7.5 * tourn
        tempy1 = y
        
        x,y = trace(net,"c","d",(x,y),(x - (random.random()*90 + 40), y ))
        #x, y= x-7.5 , y -7.5 * tourn
        
        x,y = trace(net,"d","e",(x,y),(x , y + (tourn*random.random()*90 + 40))) 
        #x, y= x+7.5 , y -7.5 * tourn
        x,y = trace(net,"e","f",(x,y),(x - (random.random()*20 + 40), y ))
        #x, y= x+7.5 , y -7.5 * tourn
        x,y = trace(net,"f","g",(x,y),(x, tempy1 ))
        #x, y= x+7.5 , y -7.5 * tourn
        x,y = trace(net,"g","h",(x,y),(x - (random.random()*20 + 40), y ))
        #x, y= x+7.5 , y -7.5 * tourn
        x,y = trace(net,"h","i",(x,y),(x, -50 ))
        #x, y= x-7.5 , y -7.5 * tourn
        
        trace(net,"i","a",(x,y),(100, -50 ))
    ...
    def _make_vehicles(self) -> None:
        ...
        """véhicules rajoutable, à voir une fois qu'on a un peu entrainné l'ia"""
         ...
         #Front vehicle
        '''
        vehicle = IDMVehicle.make_on_lane(self.road, ("c", "d", lane_index[-1]),
                                           longitudinal=rng.uniform(
                                               low=0,
                                               high=self.road.network.get_lane(("c", "d", 0)).length
                                           ),
                                           speed=9.5) #6+rng.uniform(high=3)
