...
class KinematicObservation(ObservationType):
    ...
    def observe(self) -> np.ndarray:
        ...
        # Flatten
        print(self.observer_vehicle.lane.on_lane(self.observer_vehicle.position))
...
class OccupancyGridObservation(ObservationType):
    ...
    def observe(self) -> np.ndarray:
        ...
        if self.absolute:
            ...
            print(self.env.road.vehicles[1].lane.on_lane(self.env.road.vehicles[1].position))
            return obs
...
class LidarObservation(ObservationType):
    ...
    def __init__(self, env,
        ...
        self.origin = None
        self.align_to_vehicule_axes = align_to_vehicle_axes
        

    def space(self) -> spaces.Space:
        high = 1 if self.normalize else self.maximum_range
        #print(spaces.Box(shape=(self.cells, 1), low=-high, high=high, dtype=np.float32))
        return spaces.Box(shape=(self.cells, 1), low=-high, high=high, dtype=np.float32)

    def observe(self) -> np.ndarray:
        #obs = self.trace(self.observer_vehicle.position, self.observer_vehicle.velocity)[0].copy
        
        #if self.normalize:
         #   obs /= self.maximum_range
        #print(obs)
        #modif
        obs = self.lidarview(self.cells-1)
        #print(obs)
        #print(self.env.road.vehicles[1].lane.on_lane(self.env.road.vehicles[1].position))
        #modif
        ...
        return obs
    #perso
    def lidarview(self,n):
        orient =self.observer_vehicle.heading
        reference, dist = self.trace(self.observer_vehicle.position, self.observer_vehicle.velocity)[1]
        ...
        ind1 , ind2 , _ = self.observer_vehicle.lane_index
        ind1 , ind2 = ord(ind1) -97 ,ord(ind2)  - 95
        lanelist = []
        k=0
        for w in self.env.road.network.graph.values():
                        for j in w.values():
                            k+=1
                            lanelist.append(j)
        #print(lanelist)
        ...
        for w in range(len(listealpha)):
            ...
            while res and i<30 :
                ...
                for j in range(len(visiblelane)):
                    if visiblelane[j].on_lane(self.observer_vehicle.position + i*angle):
                                res=True
                                break
        ...
        if crash == 0:
            #print("oui")
            self.observer_vehicle.crashed = True
        if self.observer_vehicle.checkpoint > 3 :
            self.observer_vehicle.crashed = True
        self.grid = affich
        
        vit = np.cos(self.observer_vehicle.velocity[0]) + np.sin(self.observer_vehicle.velocity[1])
        listepts.append([vit])
        return listepts
    ...
    def trace(self, origin: np.ndarray, origin_velocity: np.ndarray) -> np.ndarray:
        ...
        self.grid = np.ones((360)) * self.maximum_range
        #print(self.lidarview(5))
        

        angles = []
        min_angle,max_angle=0,0
        distv=self.maximum_range[0]
        ...
        for obstacle in self.env.road.vehicles + self.env.road.objects :
            ...
            if start < end:
                ...
                indexes = np.hstack([np.arange(start, 360), np.arange(0, end + 1)])
        ...
        #print( angles)
        return self.grid , ((min_angle,max_angle), distv)
    ...
    def angle_to_index(self, angle: float) -> int:
        return int(np.floor(angle / self.angle)) % (360)
